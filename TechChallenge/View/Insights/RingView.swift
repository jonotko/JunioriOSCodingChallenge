...
struct RingView: View {
   // let transactions: [TransactionModel]
    ...
    @State var lastOffSet: Double = 0.0
    
    private func ratio(for categoryIndex: Int) -> Double {
        // TODO: calculate ratio for each category according to cummulative expense
        // sum for all
        let sumForAllTransactions = transactionVM.sumTransactions(with: nil)

        // sum for category
        let sumForCategory = transactionVM.sumTransactions(with: Category[categoryIndex])

        return Double(sumForCategory)!/Double(sumForAllTransactions)!
    ...
    private func offset(for categoryIndex: Int) -> Double {
        ...
        for index in 0..<categoryIndex {
            lastOffSet += ratio(for: index)
        }
